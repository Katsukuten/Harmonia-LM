...
def main():
    ...
    try:
        ...
        if primer_midi_path and Path(primer_midi_path).exists():
            ...
            ids = (
                primer_tokens[0].ids
                if hasattr(primer_tokens[0], "ids")
                else primer_tokens[0]
            )
        ...
        print(
            f"Launching generation... (temp={temperature}, p={top_p}, k={k}, rep={repetition_penalty})"
        )
        ...
        if GENERATE_HEATMAP:
            ...
            try:
                with torch.no_grad():
                    attn_outputs = model.model(
                        torch.tensor([generated_ids]).to(device), output_attentions=True
                    )
                ...
                sns.heatmap(
                    ...
                    cmap="magma",
                    vmin=0,
                    vmax=float(cropped_attention.max()),
                )
                plt.title(
                    f"Causal Attention Heatmap (Last Layer, Last {window} tokens)"
                )
