...
def main():
    ...
    try:
        ...
        ids, labels, colors = [], [], []
        color_map = {"Velocity": "green", "Duration": "red", "TimeShift": "orange", "Special": "gray"}
        ...
        for token_str, token_id in vocab.items():
            ...
            if token_str.startswith("Pitch_"):
                try:
                    ...
                    labels.append("Pitch (Other)"); colors.append("blue")
                    
            elif token_str.startswith("Velocity"): 
                labels.append("Velocity"); colors.append(color_map["Velocity"])
            elif token_str.startswith("Duration"): 
                labels.append("Duration"); colors.append(color_map["Duration"])
            elif token_str.startswith("TimeShift"): 
                labels.append("TimeShift"); colors.append(color_map["TimeShift"])
            else: 
                labels.append("Special"); colors.append(color_map["Special"])
        ...
        for label in set(labels):
            idx_list = [i for i, l in enumerate(labels) if l == label]
            if N_DIM == 2:
                ax.scatter(proj[idx_list, 0], proj[idx_list, 1], c=[colors[i] for i in idx_list], label=label, alpha=0.8, edgecolors='w', linewidth=0.5)
            else:
                ax.scatter(proj[idx_list, 0], proj[idx_list, 1], proj[idx_list, 2], c=[colors[i] for i in idx_list], label=label, alpha=0.8, edgecolors='w', linewidth=0.5, s=40)
                
        # Add text for annotated notes (Octave 4)
        for idx, note_name in zip(notes_to_annotate, annotations):
            if N_DIM == 2:
                ax.text(proj[idx, 0], proj[idx, 1], note_name, fontsize=9, color='black', weight='bold', 
                        bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=1))
            else:
                ax.text(proj[idx, 0], proj[idx, 1], proj[idx, 2], note_name, fontsize=9, color='black', weight='bold',
                        bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=1))
