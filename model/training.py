...
BATCH_SIZE, ACC_GRAD = (
    8,
    16,
)  # Effective Batch Size = 128, change depending on your hardware
STEPS, LR = 60000, 6e-4  # Arbitrary values, tweak only if you know what you're doing
NUM_WORKERS = (
    0  # Set a reasonable number of your CPU's threads to use, disabled in my case
)
...
class Qwen3MusicModel(pl.LightningModule):
    def __init__(
        ...
        # Uncomment ONLY during inference if you plan to create a heatmap (since sdpa never actually
        # materializes or returns the raw attention weight matrix)
    ...
    def configure_optimizers(self):
        ...
        warmup_steps = int(
            0.03 * total_steps
        )  # Change if you want the warmup to be longer/shorter.
        ...
        cosine_decay_lrs = CosineAnnealingLR(
            optimizer, T_max=max(total_steps - warmup_steps, 1), eta_min=1e-6
        )

        # Failsafe to avoid scheduler crash if warmup_steps == 0.
        milestone = (
            max(1, round(warmup_steps / total_steps) + 1) if total_steps > 0 else 1
        )
        scheduler = SequentialLR(
            optimizer, schedulers=[warmup_lrs, cosine_decay_lrs], milestones=[milestone]
        )
...
def main():
    ...
    train_files = list((DATABASE / "midi_train").rglob("*.mid")) + list(
        (DATABASE / "midi_train").rglob("*.midi")
    )
    val_files = list((DATABASE / "midi_val").rglob("*.mid")) + list(
        (DATABASE / "midi_val").rglob("*.midi")
    )
    ...
    if (
        ...
        and list(checkpoint_dir.glob("*.ckpt"))
    ):
        ckpt_path = str(
            max(checkpoint_dir.glob("*.ckpt"), key=lambda x: x.stat().st_mtime)
        )
        resume = True
    ...
    NUM_EPOCHS = (
        200
        if SANITY_CHECK
        else round(STEPS * EFFECTIVE_BATCH_SIZE / max(1, DATASET_SIZE)) + 1
    )
    ...
    logger = TensorBoardLogger(
        str(ROOT_LOG_DIR), name="sanity_heck" if SANITY_CHECK else "Qwen3"
    )
    ...
    if SANITY_CHECK:
        ...
        callbacks_list = [
            ...
            InitialConditionAnalysis(
                output_file=OUTPUT_DIR / "conditioning_report.txt"
            ),  # See respective file for more info.
        ]
    ...
    trainer = pl.Trainer(
        ...
        if SANITY_CHECK
        else 1,  # Note: To validate mid-epoch, add the 'val_check_interval=0.5' argument instead.
